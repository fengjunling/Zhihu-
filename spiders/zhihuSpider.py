...
class ZhihuSpider(scrapy.Spider):
    ...
    username = '15937829283'
    password = 'zhihumima1234'
    ...
    def start_requests(self):
        ...
        is_login = self.login(captcha_lang='en', load_cookies=True)
        if is_login:
            start_url = 'https://www.zhihu.com/topic/19776749/organize/entire'
            return scrapy.Request(url=start_url, headers=)
    ...
    def login(self, captcha_lang: str = 'en', load_cookies: bool = True):
        ...
        if load_cookies and self.load_cookies():
            ...
            if self.check_login():
                ...
                response = self.session.get("https://www.zhihu.com/topic/19776749/organize/entire", headers=self.session.headers, cookies=self.session.cookies)
    ...
    def check_login(self):
        ...
        yield scrapy.Request(login_url, callback=self.parseTopic)
    ...
    def get_captcha(self, headers):
        ...
        # 这里必须先把参数 POST 验证码接口
        self.session.post(captcha_url, data={'input_text': capt})
        return capt
        # with open('captcha.jpg', 'wb') as f:
        #     f.write(r.content)
        # captcha = input("验证码：")
